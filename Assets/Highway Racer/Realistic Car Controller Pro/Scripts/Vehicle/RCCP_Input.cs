...
public class RCCP_Input : RCCP_Component {
    ...
    private void Update() {
        ...
        float angle = HR_CalculateAngle.CalculateAngle(transform.rotation, Quaternion.identity * Quaternion.AngleAxis(CarController.FrontAxle.maxSteerAngle * (steerInput * steeringCurve.Evaluate(CarController.speed)), Vector3.up), CarController.FrontAxle.maxSteerAngle * steeringCurve.Evaluate(CarController.speed));
