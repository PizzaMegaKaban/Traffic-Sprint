...
public class HR_TrafficPooling : MonoBehaviour {
    ...
    private void AnimateTraffic() {
        ...
        for (int i = 0; i < _trafficCars.Count; i++) {
            ...
            if (Camera.main.transform.position.z > (_trafficCars[i].transform.position.z + 100) || Camera.main.transform.position.z < (_trafficCars[i].transform.position.z - 400))
    ...
    private void ReAlignTraffic(HR_TrafficCar realignableObject) {
        ...
        realignableObject.transform.position = new Vector3(lines[randomLine].position.x, lines[randomLine].position.y, (Camera.main.transform.position.z + (Random.Range(200, 400))));
